...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        const followerCountValue = userData.followersCount ?? (userData.followers ? userData.followers.length : 0);
        const followingCountValue = userData.followingCount ?? (userData.following ? userData.following.length : 0);
  ...
  // ======================================================
  useEffect(() => {
    if (!profileData || !profileData._id || !backendUrl) return;
    ...
    socket.emit("join_private_room", profileData._id);
  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const updateData = payload?.user ? payload.user : payload;

        setProfileData((prev) => ({
          ...prev,
          ...updateData,
          followersCount: updateData.followersCount ?? (updateData.followers ? updateData.followers.length : prev.followersCount),
          followingCount: updateData.followingCount ?? (updateData.following ? updateData.following.length : prev.followingCount),
        }));
    ...
    try {
      ...
      if (response.data.success) {
        setSavedPosts((prev) => {
          ...
          const postToAdd = allPosts.find((post) => post._id === postId);
  ...
  const activePosts = activeTab === "posts" ? userPosts : savedPosts;
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="grid grid-cols-3 gap-1 md:gap-4 mt-6">
          {activePosts.map((post) => {
            const isPostSaved = savedPostIds.has(post._id);
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-3 text-white font-bold">
                  ...
                  <div className="flex justify-end">
                    ...
                    >
                      <Bookmark
                        size={18}
                        className={isPostSaved ? "fill-white text-white" : "text-white"}
                      />
                    </button>
                  </div>
